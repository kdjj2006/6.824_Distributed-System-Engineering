...
* 高层次复制图
    * ```java
        ...
        o CM
        o o o ZK
* 每个区域 f+1 份拷贝来容忍每个区域 <= f 个故障
    * TCs 发送所有写操作给所有副本(TC 的 COMMIT-BACKUP)
    * 如果一个服务器崩溃不能立刻可用
        * 事务读和提交要等待
    * 但是 CM 马上会通知，新建一个副本，恢复事务
* 重新配置
    * 一个 ZooKeeper 集群(少数副本)
        * 只存储配置，此配置中的服务器集合，CM
        * 如果多个服务器试图变成 CM 则断线
        * 如果发生分区选择活性分区(主分区)
    * 一个配置管理器(CM)(没有被复制)
        * 通过快速的 ping 来监视所有服务器的活跃度
        * 管理重新配置
            * 续约
                * 仅在得到大多数计算机的响应时才激活
            * 检查每个区域至少存在一个副本
            ...
            * 管理中断交易的完成
* 让我们回顾一下图4的提交协议，看看如何
    * 尽管服务器出现故障，但可能已提交的所有 xaction 仍然可见
    * “可能已提交”
        * TC 可能已经给客户端回复了"yes"
        * 主服务器可能已经表明了对后续阅读的更新
* 在 TC 看见从所有来自 LOCKs 和 VALIDATEs 的 "yes"
    * TC 给每个备份的日志追加 COMMIT-BACKUP
    * 在所有 ack 确认之后，给每个主服务器的日志追加 COMMIT-PRIMARY
    * 在一个 ack 确认后，给应用报告"提交"
* 注意 TC 复制到备份，主服务器不复制
    * COMMIT-BACKUP 包括写的值，足够更新备份机的状态
* 为什么 TC 只在收到所有 COMMIT-BACKUP 的确认之后才发送 COMMIT-PRIMARY ？
    * 一个主服务器可能在看见 COMMIT-PRIMARY 之后立刻执行
        * 并且将更新展现给其他的事务
    * 因此，到那时，每个对象的新值必须在 f+1 个日志中(每个区域)
        * 因此 f 可以故障而不丢失新值
    * 如果有一个备份没有 COMMIT-BACKUP
        * 对象写入只存在 f 个日志中
        * 所有 f 都可能与 TC 一起失败
        * 那么我们本可公布此提交，但可能永久丢失一次写入
* 为什么 TC 要等待来自 COMMIT-PRIMARY 的一个确认？
    * 这样就可以知道提交的完整 f+1 区域
    * 在那之前，每个区域只有 f 个备份知道(来自 COMMIT-BACKUP)
    * 但是我们假设每个区域的 f 个服务器故障
* 为什么可以进行恢复的基本理由
    * 如果TC可能报告了“提交”，或者主服务器可能已经公布了新值
    * 然后每个区域的所有 f+1 在日志中有了 LOCK 或者 COMMIT-BACKUP
    * 所有 f 可以从任何/所有分区中宕机而没有丢失写入
    * 如果恢复看到了一个或多个 COMMIT-*,一个 COMMIT-*,或者 LOCKd
        * 从每个区域，提交；否则中止
        * 也就是表明 TC 决定提交，加上每个多谢的写
        * (5.3 章节，第7步)
* FaRM 很好，它不完美吗？
    * 由于 OCC，如果冲突很少，则效果最佳
    * 数据必须适合总 RAM
    * 数据模型是低级的；将需要例如 SQL 库
