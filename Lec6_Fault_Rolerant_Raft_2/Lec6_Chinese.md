...
* 崩溃和重启过程中会发生什么？
    ...
    * Raft 从磁盘读取持久化日志
        * 给服务发送已提交但不在快照中的日志条目
* 如果追随者落后并且领导者已经放弃了追随者日志的结尾，该怎么办？
    * nextIndex[i] 会回到领导者日志的开始
    * 所以领导者不能通过 AppendEntries RPCs 修复那个追随者
    * 因此使用 InstallSnapshot RPC      
    (问题：为什么领导者不丢弃所有服务器都有的日志条目呢？)
* 在一个 InstallSnapshot RPC 里面有什么呢？图12，13
    * 任期
    * lastIncludedIndex
    * lastIncludedTerm
    * 快照数据
* 一个追随者会针对 InstallSnapshot 做什么？
    * 拒绝如果任期是旧的(不是当前任期的领导者)
    * 拒绝(忽略)如果追随者已经包含索引/任期 
        * 是一个旧的/延迟的 RPC
    * 清空日志，用假的上一个条目代替
    * 将 lastApplied 设置为 lastIncludedIndex
    * 用快照内容替换服务器状态(例如 k/v table)
* 请注意，状态和操作历史大致相同
    * 设计者可以选择发送哪个
    * 例如最近的几个操作(日志条目)给落后的副本
        * 但是丢失其磁盘的副本的整个状态（快照）。
    * 另外，修复副本可能很昂贵，需要注意
* 问题：
    * 收到的 InstallSnapshot RPC 是否会导致状态机运行及时倒退？也就是图13中的第八步能不能导致状态机重启然后影响一些已经执行的操作？如果是的，解释如何发生，如不能，解释为什么不能发生
### 主题：配置改变(实验不需要)
* 配置变化(第6章)
    * 配置 = 一组服务器
    * 有时你需要
...
* Raft 配置变化
    ...
    * 系统以 Cold 开始
    * 系统管理员要求领导者切到 Cnew
    * Raft 有特殊的配置日志条目(服务器地址集)
    * 每个服务器在自己的日志中使用最后一个配置
        *  1.领导者提交 Cold，对 Cold 和 Cnew 的大多数都是新手
        *  2.在 Cold 之后，新的提交，领导者提交 Cnew 给在 Cnew 中的服务器
* 如果领导者在这个过程中的各个方面崩溃了怎么办？
    * 在下个任期中我们可以有两个领导者吗？
    * 如果这个发生了，每个领导者必须是下面中的一个：
        * A. 在 Cold 中，但没有 Cold，新的日志
        * B. 在 Cold 或者 Cnew 中，有 Cold，新的日志
        * C. 在 Cnew 中，在日志中有 Cnew
    * 我们知道我们在领导选举的一般规则下不会又 A+A 或者 C+C
        * A+B? 不，因为 B 需要 Cold 和 Cnew 中的大多数
        * A+C? 不，因为直到 Cold 才能进入 Cnew,新的提交给 Cold
        * B+B? 不，因为 B 需要 Cold 和 Cnew 两者中的大多数
        * B+C? 不，因为 B 需要 Cold 和 Cnew 中的大多数
* 很好。Raft 可以切换到没有两个活动领导者风险的新服务器集合中
### 主题：性能
* 注意：很多情况不需要高性能
    * key/value 存储可能需要
    * 但是 GFS 或者 MapReduce master 可能不需要
* 大多数复制系统拥有类似的常见性能：
    * 每个协议一次RPC交换和一次磁盘写入
    * 所以 Raft 对于消息复杂度来说是一个典型
* Raft 做出了一些设计选择，为了简单起见牺牲了性能：
    * 追随者拒绝过期的 AppendEntries RPC
        * 而不是存下来当洞被填充之后再使用
        * 如果网络重排序包很多的时候很重要
    * 没有批量或流水线 AppendEntries 的规定
    * 快照对大型状态来说很浪费
    * 一个缓慢的领导者可能损害 Raft
* 这些多性能有很大影响
    * 磁盘写入持久化
    * Message/packet/RPC 开销
    * 需要顺序执行已记录的命令
    * 只读操作的快速通道
* 更关注性能的论文
